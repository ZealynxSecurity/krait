...
async function searchSolodit(filters) {
    ...
    if (filters.impact?.length)
        apiFilters.impact = filters.impact.map((i) => i.toUpperCase());
    ...
    const findings = (json.findings ?? []).map((f) => ({
        ...
        tags: f.tags ?? [],
        body: f.body ?? "",
        protocolCategory: f.protocolCategory ?? "",
 ...
 */
server.tool("search_similar_findings", "Search Solodit for real audit findings similar to a suspected vulnerability. Returns matching exploits from past audits with titles, impact, descriptions, and URLs.", {
    ...
    try {
        ...
        if (findings.length === 0) {
            return { content: [{ type: "text", text: `No Solodit findings found for: "${query}"` }] };
        }
        ...
        return { content: [{ type: "text", text: `Solodit search failed: ${msg}` }] };
    }
});
/**
 ...
 */
server.tool("get_enrichment", "Fetch high-quality Solodit findings for a specific protocol type. Returns real exploits as context for analysis.", {
    ...
    try {
        ...
        if (findings.length === 0) {
            return { content: [{ type: "text", text: `No Solodit findings found for protocol type: "${protocol_type}"` }] };
        }
        ...
        const msg = e instanceof Error ? e.message : String(e);
        return { content: [{ type: "text", text: `Solodit enrichment failed: ${msg}` }] };
    }
});
/**
 ...
 */
server.tool("validate_hypothesis", "Check if a vulnerability hypothesis has historical precedent on Solodit. Returns the number of similar findings and their details. High match count = higher confidence the bug is real.", {
    ...
    try {
        ...
        const msg = e instanceof Error ? e.message : String(e);
        return { content: [{ type: "text", text: `Validation failed: ${msg}` }] };
    }
});
// ── Start ──
async function main() {
